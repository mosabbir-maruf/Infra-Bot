  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "*"
        ...
        <p><strong>Generate a Client Secret:</strong> In the app registration → Certificates &amp; secrets → Client secrets → New client secret. Add description and expiration, then <strong>copy the secret value immediately</strong> — this is <code>AZURE_CLIENT_SECRET</code>. Azure displays two fields: a <strong>Secret ID</strong> (identifier) and a <strong>Value</strong> (the actual secret). <em>Make sure to copy the Value, not the Secret ID — the value is a long random string, not a UUID.</em></p>
